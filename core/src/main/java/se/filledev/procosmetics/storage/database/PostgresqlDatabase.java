...
public class PostgresqlDatabase extends SQLDatabase {
    ...
    protected String getAddGadgetAmmo() {
        // The target table is aliased because the query is formatted with a single table name argument
        return "INSERT INTO %s AS t (player_id, gadget, amount) VALUES (?, ?, ?) " +
                "ON CONFLICT (player_id, gadget) DO UPDATE SET amount = t.amount + EXCLUDED.amount;";
    }
    ...
    protected String getAddTreasureChests() {
        // The target table is aliased because the query is formatted with a single table name argument
        return "INSERT INTO %s AS t (player_id, treasure_chest, amount) VALUES (?, ?, ?) " +
                "ON CONFLICT (player_id, treasure_chest) DO UPDATE SET amount = t.amount + EXCLUDED.amount;";
    }
